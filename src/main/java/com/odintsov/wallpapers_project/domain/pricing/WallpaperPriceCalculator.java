...
import com.odintsov.wallpapers_project.domain.entities.OrderItemExtraFeature;
...
public class WallpaperPriceCalculator implements PriceCalculator {
    ...
    public Double calculate(BaseProduct product, OrderItem item) {
        double width = Double.parseDouble(item.getSpecifications().get(WallpaperFields.WIDTH));
        double height = Double.parseDouble(item.getSpecifications().get(WallpaperFields.HEIGHT));
        double area = (width / 100.0) * (height / 100.0);
        double baseUnitPrice = product.getPrice() * area;

        double featuresPricePerUnit = 0.0;
        if (item.getOrderItemExtraFeatures() != null) {
            for (OrderItemExtraFeature link : item.getOrderItemExtraFeatures()) {
                if (link.getExtraFeature() != null) {
                    featuresPricePerUnit += link.getExtraFeature().getPrice();
                }
            }
        }
